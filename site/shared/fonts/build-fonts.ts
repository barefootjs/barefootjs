/**
 * Regenerate inter-bold.ts from inter-bold.ttf as a base64-encoded
...
const TTF_PATH = new URL('./inter-bold.ttf', import.meta.url).pathname
const TS_PATH = new URL('./inter-bold.ts', import.meta.url).pathname

const ttf = await Bun.file(TTF_PATH).arrayBuffer()
const b64 = Buffer.from(ttf).toString('base64')

const ts = `/**
 * Inter Bold (Latin subset) — generated from inter-bold.ttf.
 * Regenerate via: bun run site/shared/fonts/build-fonts.ts
 */
const BASE64 = '${b64}'
export const interBoldFont = Uint8Array.from(atob(BASE64), c => c.charCodeAt(0))
`
