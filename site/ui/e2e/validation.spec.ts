...
test.describe('Form Validation Documentation Page', () => {
  ...
  test.describe('Required Field Demo', () => {
    test('displays required field demo', async ({ page }) => {
      await expect(page.locator('[bf-s^="RequiredFieldDemo_"][bf-r]')).toBeVisible()
    })

    test('shows no error initially', async ({ page }) => {
      const demo = page.locator('[bf-s^="RequiredFieldDemo_"][bf-r]')
  ...
  test.describe('Email Validation Demo', () => {
    test('displays email validation demo', async ({ page }) => {
      await expect(page.locator('[bf-s^="EmailValidationDemo_"][bf-r]')).toBeVisible()
    })

    test('shows required error when empty on blur', async ({ page }) => {
      const demo = page.locator('[bf-s^="EmailValidationDemo_"][bf-r]')
      const input = demo.locator('input')
      const error = demo.locator('.error-message')
  ...
  test.describe('Password Confirmation Demo', () => {
    test('displays password confirmation demo', async ({ page }) => {
      await expect(page.locator('[bf-s^="PasswordConfirmationDemo_"][bf-r]')).toBeVisible()
    })

    test('shows password length error', async ({ page }) => {
      const demo = page.locator('[bf-s^="PasswordConfirmationDemo_"][bf-r]')
  ...
  test.describe('Multi-Field Form Demo', () => {
    test('displays multi-field form demo', async ({ page }) => {
      await expect(page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r]')).toBeVisible()
    })
    ...
    test('shows all field errors on submit with empty form', async ({ page }) => {
      ...
      const demo = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r]')
      await expect(demo).toBeVisible()

      const submitButton = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] button')
      const nameError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .name-error')
      const emailError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .email-error')
      const passwordError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .password-error')
      const confirmError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .confirm-error')
      const formError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .form-error')
    ...
    test('shows name length error for short name', async ({ page }) => {
      ...
      const nameInput = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] input').first()
      const nameError = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .name-error')
    ...
    test('submits successfully with valid data', async ({ page }) => {
      ...
      const inputs = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] input')
      const nameInput = inputs.nth(0)
      const emailInput = inputs.nth(1)
      const passwordInput = inputs.nth(2)
      const confirmInput = inputs.nth(3)
      const submitButton = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] button')
      const successMessage = page.locator('[bf-s^="MultiFieldFormDemo_"][bf-r] .success-message')
  ...
  test.describe('Async Availability Demo', () => {
    const scope = '[bf-s^="AsyncFieldValidationDemo_"][bf-r]:not([data-slot])'
