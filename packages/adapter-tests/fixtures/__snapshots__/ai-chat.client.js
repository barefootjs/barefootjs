...
export function initAIChatInteractive(__scope, _p = {}) {
  ...
  mapArray(() => messages(), _s5, (msg) => String(msg.id), (msg, __idx, __existing) => {
    const __el = __existing ?? (() => { const __tpl = document.createElement('template'); __tpl.innerHTML = `<div data-key="${msg().id}" ${(`chat-msg chat-${msg().role}`) != null ? 'class="' + (`chat-msg chat-${msg().role}`) + '"' : ''} bf="s1"><div class="chat-bubble"><p><!--bf:s0-->${msg().content}<!--/--></p></div></div>`; return __tpl.content.firstElementChild.cloneNode(true) })()
...
hydrate('AIChatInteractive', { init: initAIChatInteractive, template: (_p) => `<div class="chat-container"><div class="chat-messages" id="chat-messages" bf="s5"><!--bf-loop:l0-->${([]).map((msg) => `<div data-key="${msg.id}" ${(`chat-msg chat-${msg.role}`) != null ? 'class="' + (`chat-msg chat-${msg.role}`) + '"' : ''} bf="s1"><div class="chat-bubble"><p><!--bf:s0-->${msg.content}<!--/--></p></div></div>`).join('')}<!--bf-/loop:l0-->${(false) ? `<div bf-c="s2" class="chat-msg chat-assistant"><div class="chat-bubble"><p bf="s4"><!--bf:s3-->${('')}<!--/--><span class="streaming-cursor">▌</span></p></div></div>` : `<!--bf-cond-start:s2--><!--bf-cond-end:s2-->`}</div><div class="chat-input-area"><input type="text" class="chat-input" placeholder="Type a message..." ${(('')) != null ? 'value="' + (('')) + '"' : ''} ${(false) ? 'disabled' : ''} bf="s6" /><button class="chat-send" ${(false) ? 'disabled' : ''} bf="s7"> Send </button></div></div>` })
