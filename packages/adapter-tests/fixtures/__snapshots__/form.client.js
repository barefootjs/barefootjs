import { $, createComponent, createEffect, createSignal, escapeAttr, hydrate, insert, styleToCss } from '@barefootjs/client/runtime'
...
hydrate('Form', { init: initForm, template: (_p) => `<div class="form-container" style="padding: 24px; max-width: 400px; margin: 0 auto;"><h2 style="margin-top: 0;">Terms and Conditions</h2><p style="color: #666; font-size: 14px;"> Please read and accept the terms before continuing. </p><div class="checkbox-row" style="display: flex; align-items: center; gap: 12px; margin: 20px 0;"><button class="checkbox" ${(`${(false) ? 'checked' : 'unchecked'}`) != null ? 'data-state="' + escapeAttr(`${(false) ? 'checked' : 'unchecked'}`) + '"' : ''} ${((v) => v != null ? 'style="' + escapeAttr(v) + '"' : '')(styleToCss(`width: 24px; height: 24px; border: 2px solid ${(false) ? '#4caf50' : '#ccc'}; border-radius: 4px; background: ${(false) ? '#4caf50' : 'white'}; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0;`))} ${((false)) != null ? 'aria-checked="' + escapeAttr((false)) + '"' : ''} role="checkbox" bf="s1">${(false) ? `<svg bf-c="s0" class="checkmark" width="16" height="16" viewBox="0 0 16 16" fill="none" style="display: block;"><path d="M3 8L6.5 11.5L13 5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>` : `<!--bf-cond-start:s0--><!--bf-cond-end:s0-->`}</button><label class="checkbox-label" style="cursor: pointer; user-select: none;"> I agree to the terms and conditions </label></div><button class="submit-btn" ${!(false) ? 'disabled' : ''} ${((v) => v != null ? 'style="' + escapeAttr(v) + '"' : '')(styleToCss(`width: 100%; padding: 12px 24px; font-size: 16px; border: none; border-radius: 6px; cursor: ${(false) ? 'pointer' : 'not-allowed'}; background: ${(false) ? '#4caf50' : '#e0e0e0'}; color: ${(false) ? 'white' : '#999'};`))} bf="s2"> Continue </button></div>` })
