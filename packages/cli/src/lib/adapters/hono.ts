...
// are hand-written design tokens — so the entries below name only the
// generated children (compiled SSR templates, hashed client JS,
// regenerated uno.css, the build cache, wrangler's local state, the
// per-machine path entry npm writes into package-lock.json on
// pkg.pr.new tarball installs).
const HONO_GITIGNORE = `# Dependencies
node_modules/

# Wrangler local state (dev session, worker logs)
.wrangler/

# bf build outputs (regenerated by \`bf build\` / \`bf build --watch\`)
public/components/
